...
class GlobalSearch(BaseSearch):
    ...
    async def asearch(
        ...
        **kwargs: Any,
        ...
        reduce_response = await self._async_reduce_response(
    ...
    def search(
        ...
        streaming: bool = True,
        **kwargs: Any,
    ) -> GlobalSearchResult | Generator[str, None, GlobalSearchResult]:
        ...
        if self.callbacks:
            for callback in self.callbacks:
                callback.on_map_response_start(context_chunks)  # type: ignore
        map_responses = [
            asyncio.run(self._map_response_single_batch(
                context_data=data, query=query, **self.map_llm_params
            )) for data in context_chunks
        ]
        if self.callbacks:
        ...
        if streaming:
            while True:
                try:
                    yield reduce_response.__next__()
                except StopIteration as e:
                    resp: SearchResult = e.value
                    return GlobalSearchResult(
                        response=resp.response,
                        context_data=context_records,
                        context_text=context_chunks,
                        map_responses=map_responses,
                        reduce_context_data=resp.context_data,
                        reduce_context_text=resp.context_text,
                        completion_time=time.time() - start_time,
                        llm_calls=map_llm_calls + resp.llm_calls,
                        prompt_tokens=map_prompt_tokens + resp.prompt_tokens,
                    )
        else:
            try:
                reduce_response.__next__()
            except StopIteration as e:
                reduce_response = e.value
            return GlobalSearchResult(
                response=reduce_response.response,
                context_data=context_records,
                context_text=context_chunks,
    ...
    async def _async_reduce_response(
        self,
        map_responses: list[SearchResult],
        query: str,
        **llm_kwargs,
    ...
    def _reduce_response(
        ...
        **llm_kwargs,
        ...
        try:
            ...
            search_response = self.llm.generate(
                search_messages,
                streaming=streaming,
                callbacks=self.callbacks,  # type: ignore
                **llm_kwargs,  # type: ignore
            ...
            if streaming:
                while True:
                    try:
                        yield search_response.__next__()
                    except StopIteration as e:
                        return SearchResult(
                            response=e.value,
                            context_data=text_data,
                ...
                try:
                    search_response.__next__()
                except StopIteration as e:
                    search_response = e.value
                return SearchResult(
                    response=search_response,
                    context_data=text_data,
                    context_text=text_data,
