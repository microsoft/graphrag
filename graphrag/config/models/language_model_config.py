...
class LanguageModelConfig(BaseModel):
    ...
    def _validate_tokens_per_minute(self) -> None:
        ...
        if isinstance(self.tokens_per_minute, int) and self.tokens_per_minute < 1:
            msg = f"Tokens per minute must be a non zero postive number, 'auto' or null. Suggested value: {language_model_defaults.tokens_per_minute}."
    ...
    def _validate_requests_per_minute(self) -> None:
        ...
        if isinstance(self.requests_per_minute, int) and self.requests_per_minute < 1:
            msg = f"Requests per minute must be a non zero postive number, 'auto' or null. Suggested value: {language_model_defaults.requests_per_minute}."
            raise ValueError(msg)
