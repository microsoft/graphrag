...
def normalize_conversation_metadata(
    row: dict, turn_counters: dict[str, int] | None = None
) -> dict:
    ...
    if turn_timestamp is None or turn_timestamp == "":
    ...
    if turn_role is None or turn_role == "":
        turn_role = "unknown"
    row["turn_role"] = turn_role
    return row


def _get_source_identifier(row: dict) -> str:
