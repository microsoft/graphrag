...
internal static class CreateFinalDocumentsWorkflow
{
    ...
    public static WorkflowDelegate Create()
    {
        return async (config, context, cancellationToken) =>
        {
            ...
            var textUnits = await context.OutputStorage.LoadTableAsync<TextUnitRecord>("text_units", cancellationToken).ConfigureAwait(false);
            ...
            await context.OutputStorage.WriteTableAsync("documents", finalized, cancellationToken).ConfigureAwait(false);
