...
from comparision_prompt import eval_system_prompt,eval_user_prompt, RAG_SYSTEM_PROMPT, RESPONSE_TYPE
...
class GraphRagComparision:
    ...
    def __init__(self, root_dir: str, temperature: float = 0.0):
        ...
        self.compare_dir = self.root_dir.joinpath("comparision_results")
    ...
    def plot_comparision_results(self,):
        ...
        #plt.show()
        plt.gcf().savefig(self.compare_dir.joinpath('comparision_plot.png'), bbox_inches='tight')
        
        
    def print_comparision_results(self,):
        ...
        img = plt.imread(self.compare_dir.joinpath('comparision_plot.png'))
        plt.imshow(img)
        plt.axis('off') 
        plt.show()
...
if __name__ == "__main__":
    ...
    async def main():
            graph_rag_comparision = GraphRagComparision(root_dir=ROOT_DIR, temperature=TEMPERATURE_SETTING)

            evaluation_results = await graph_rag_comparision.answer_questions(
                questions=questions_to_evaluate,
                sleep_interval=5 
            )
            print(evaluation_results)
            graph_rag_comparision.compare_answers(evaluation_results,
                                                         )
            print(f"Evaluation results saved to {graph_rag_comparision.compare_dir}.")
            
            graph_rag_comparision.plot_comparision_results()
