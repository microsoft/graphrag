...
# Authenticate into Azure Artifacts using a user PAT. (Needs artifact read permissions)
RESILIENCE_ARTIFACTS_PASSWORD="<PAT>" yarn python_authenticate

# Install Python dependencies.
yarn python_install 

# Execute the Indexing Engine
yarn run:index --root=<data_project_directory> # default config mode
yarn run:index --config=<config_file_path> # custom config mode
```
```
