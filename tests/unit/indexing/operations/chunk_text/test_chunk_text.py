...
def test_chunk_text_metadata(
    ...
    chunk_text(
        input_data, column, size, overlap, encoding_model, strategy, callbacks, metadata
    )
...
@mock.patch("graphrag.index.operations.chunk_text.chunk_text.run_strategy")
@mock.patch("graphrag.logger.progress.ProgressTicker")
def test_chunk_text_metadata_line_delimiter(
    mock_progress_ticker, mock_run_strategy, mock_load_strategy
):
    input_data = pd.DataFrame({
        "name": ["The Shining"],
...
@mock.patch("graphrag.index.operations.chunk_text.chunk_text.run_strategy")
@mock.patch("graphrag.logger.progress.ProgressTicker")
def test_chunk_text_multiple_metadata(
    mock_progress_ticker, mock_run_strategy, mock_load_strategy
):
    input_data = pd.DataFrame({
        "name": ["The Shining", "The Great Gatsby"],
        "author": ["Stephen King", "F. Scott Fitzgerald"],
        "type": ["book", "movie"],
        "genre": ["horror", "drama"],
    })
    column = "name"
    ...
    assert mock_run_strategy.call_args_list == expected_calls
