...
def test_init_with_default_log_file_name(s3_bucket):
    ...
    with patch(
        "graphrag.callbacks.s3_workflow_callbacks._get_log_file_name"
    ) as mock_get_log_file_name:
...
def test_log_message(s3_callbacks, s3_mock, bucket_name):
    ...
    with patch("graphrag.callbacks.s3_workflow_callbacks.datetime") as mock_datetime:
        ...
        response = s3_mock.list_objects_v2(
            Bucket=bucket_name, Prefix="test-logs/test-log.json"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("log", LogType.LOG, "Info message", "log", {"details": {"key": "value"}}),
    ],
)
...
def test_write_log_client_error(s3_callbacks):
    ...
    # Patch the create_s3_client function where it's imported
    with patch(
        "graphrag.callbacks.s3_workflow_callbacks.create_s3_client"
    ) as mock_create_s3_client:
        mock_client_instance = MagicMock()
        error_response = {"Error": {"Code": "InternalError", "Message": "Test error"}}
        mock_client_instance.put_object.side_effect = ClientError(
            error_response, "PutObject"
        )
        mock_create_s3_client.return_value = mock_client_instance
        ...
        with patch("graphrag.callbacks.s3_workflow_callbacks.logger") as mock_logger:
            # Accessing _write_log will trigger _s3_client property, which calls create_s3_client
            s3_callbacks._write_log({"type": "log", "data": "Test message"})  # noqa: SLF001

            # Check that create_s3_client was called (due to lazy loading reset)
            mock_create_s3_client.assert_called_once()
            # Check that the error was logged because put_object failed
...
def test_init_with_aws_credentials():
    ...
    with patch(
        ...
        assert mock_create_s3_client.call_count == 0

        # Access the client to trigger lazy loading
        client = callbacks._s3_client  # noqa: SLF001

        # Verify create_s3_client was called with the correct arguments
        mock_create_s3_client.assert_called_once_with(
            endpoint_url=None,  # Explicitly check default
            aws_access_key_id="test-access-key",
            aws_secret_access_key="test-secret-key",
            region_name="us-west-2",
        )
...
def test_init_with_endpoint_url():
    ...
    with patch(
        ...
        callbacks = S3WorkflowCallbacks(
            bucket_name="test-bucket", endpoint_url=endpoint
        )

        # Verify endpoint_url is stored but client is not created yet
        assert callbacks._endpoint_url == endpoint  # noqa: SLF001
        assert mock_create_s3_client.call_count == 0
...
def test_init_with_partial_aws_credentials():
    ...
    with patch(
        ...
        # Verify create_s3_client was called without secret key
        mock_create_s3_client.assert_called_once_with(
            endpoint_url=None,
            aws_access_key_id="test-access-key",
            aws_secret_access_key=None,
...
def test_init_with_empty_endpoint_url():
    ...
    with patch(
        ...
        callbacks = S3WorkflowCallbacks(bucket_name="test-bucket", endpoint_url="")
        ...
        # Verify create_s3_client was called with empty endpoint_url
        # The utility function handles the logic of not passing it to boto3 if empty
        mock_create_s3_client.assert_called_once_with(
            endpoint_url="",
            aws_access_key_id=None,
            aws_secret_access_key=None,
...
def test_init_with_whitespace_endpoint_url():
    ...
    with patch(
        ...
        # Verify create_s3_client was called with whitespace endpoint_url
        # The utility function handles the logic of not passing it to boto3 if whitespace
        mock_create_s3_client.assert_called_once_with(
            endpoint_url=endpoint,
            aws_access_key_id=None,
            aws_secret_access_key=None,
...
def test_lazy_loading_s3_client():
    ...
    with patch(
        ...
        # Check it was called with default args
        mock_create_s3_client.assert_called_once_with(
            endpoint_url=None,
            aws_access_key_id=None,
            aws_secret_access_key=None,
        ...
        assert mock_create_s3_client.call_count == 1  # Still only called once
