...
def test_elasticsearch_text_embedder_search():
    ...
    results = vector_store.similarity_search_by_text("test query", empty_embedder, k=2)
    assert len(results) == 0

    # Test text similarity search
    results = vector_store.similarity_search_by_text(
        "Medium length query text", mock_embedder, k=2
    )
...
def test_elasticsearch_numpy_vector_handling():
    ...
    results = vector_store.similarity_search_by_text("test query", numpy_embedder, k=1)
